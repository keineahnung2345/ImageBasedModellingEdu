...
fssrecon (AppOptions const& app_opts, fssr::SampleIO::Options const& pset_opts)
{
    ...
    for (std::size_t i = 0; i < voxels.size(); ++i) {
        ...
        // index of the voxel
        fssr::VoxelIndex index = voxels[i].first;

        // calculate voxel position
        math::Vec3d voxel_pos = index.compute_position(
            octree.get_root_node_center(), octree.get_root_node_size());

        std::cout << "voxel id: " << i 
            << ", pos: " << voxel_pos 
            << ", sdf(value, conf, scale, color): "
            << voxels[i].second.value << ", "
            << voxels[i].second.conf  << ", "
            << voxels[i].second.scale << ", "
            << voxels[i].second.color << std::endl;
    }

    fssr::Octree::Iterator iter = octree.get_iterator_for_root();
    // for (iter.first_node(); iter.current != nullptr; iter.next_node()){
    //     // (unsigned int): convert uint8_t unprintable char to printable one
    //     // https://stackoverflow.com/questions/19562103/uint8-t-cant-be-printed-with-cout
    //     std::cout << "dfs path: " << std::oct << iter.path << 
    //         ", level: " << std::dec << (unsigned int)iter.level << std::endl;
    // }

    iter = octree.get_iterator_for_root();
    for (iter.first_node(); iter.current != nullptr; iter.next_bread_first()){
        std::cout << "bfs path: " << std::oct << iter.path << 
            ", level: " << std::dec << (unsigned int)iter.level << std::endl;
    }

    // change the signed distance value of voxels
    for (std::size_t i = 0; i < octree.voxels.size(); ++i) {
        octree.voxels[i].second.value -= 100;
    }
